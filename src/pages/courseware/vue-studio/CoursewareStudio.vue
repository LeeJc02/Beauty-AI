<template>
  ...
  >
    ...
    <Transition name="studio-stage" mode="out-in" @after-enter="followConversation">
      <div v-if="busy" key="loading" class="studio__body" aria-busy="true">
      ...
      <div v-else key="workspace" class="studio__body">
        <div
                  ...
                  >
                    ...
                    <div class="studio__options">
                      ...
                      >
                        ...
                        <em
                          v-if="option.recommended || question.recommendedOptionId === option.id"
                      ...
                      >
                        ...
                        />
                        <span class="studio__option-body"
                          ><strong>{{ s('noneAnswer') }}</strong></span
                        >
                      </label>
                    </div>
                    <label
        ...
        >
          <header class="studio__draft-header">
            ...
            <span
              ...
              :class="{ 'is-editable': waitingForConfirmation }"
            >
              {{ waitingForConfirmation ? s('editable') : s('fromConversation') }}
            </span>
          </header>
          <div class="studio__document">
            <div
              v-if="showGeneratedPages"
              :key="`part-${activePart?.partIndex || 0}`"
              class="studio__generated-pages"
              :class="{
                'is-page-turning': partSwitching,
            ...
            >
              ...
              >
                <div class="studio__generated-page-heading">
                  ...
                  >
                  <span
                    class="studio__page-status"
                    :class="`is-${quizCardStatus}`"
                    >{{ quizCardStatusLabel }}</span
                  >
                </div>
            ...
            <template v-else-if="draft.outlines.length">
              ...
              <details v-if="waitingForConfirmation" class="studio__page studio__planning-goals">
              ...
              <div v-for="part in outlineParts" :key="part.partIndex" class="studio__part">
                ...
                >
                  ...
                  <div v-if="draft.outlines[pageIndex(outline.id)]" class="studio__page-content">
                    <div v-if="waitingForConfirmation" class="studio__section-actions">
                      <button
                        type="button"
                    ...
                    <template
                      v-if="
                        ...
                        (editingSection === outline.id || reviewingOutline)
                      "
                    >
                      <label class="studio__input-label">
            ...
            <template v-else-if="summary && waitingForConfirmation">
    ...
    >
      <div class="studio__preview-shell" :aria-busy="previewLoading">
        <div class="studio__preview-toolbar" aria-label="页面切换">
          <button
            type="button"
            class="studio__preview-nav"
            :disabled="previewPageIndex <= 0 || previewLoading"
          ...
          <button
            type="button"
            class="studio__preview-nav"
            :disabled="previewPageIndex < 0 || previewPageIndex >= previewPageCount - 1 || previewLoading"
        ...
        <template v-else-if="preview">
          <p v-if="preview.mediaPending" class="studio__preview-notice">{{ s('mediaPending') }}</p>
          <iframe
            :srcdoc="preview.html"
            :title="previewTitle"
...
import type { CoursewareGenerationTaskVO, CoursewareStudioConfirmation } from './apiTypes'
import type {
...
import { createLocalCoursewareApi } from './localApi'
...
const { t } = useI18n()
const CoursewareApi = createLocalCoursewareApi(() => props.task)
...
let partSwitchTimer: ReturnType<typeof setTimeout> | undefined
async function openPagePreview(page: CoursewarePageStateVO) {
  ...
  try {
    ...
    if (request !== previewRequest || props.task?.id !== taskId || failed.value) return
...
watch([() => props.task?.id, () => failed.value], () => {
  previewRequest += 1
...
watch(
  () => props.task?.generation?.pages.find((page) => page.id === selectedPageId.value)?.version,
  () => {
    const page = props.task?.generation?.pages.find((page) => page.id === selectedPageId.value)
    if (
      previewOpen.value &&
      page?.status === 'completed' &&
      !previewLoading.value &&
      Boolean(preview.value) &&
      page.version !== preview.value?.version
    )
      void openPagePreview(page)
  }
)
...
const partSwitching = ref(false)
const partSwitchDirection = ref<'next' | 'previous'>('next')
const partState = (item: CoursewareGenerationSeriesItemVO) => {
  ...
  if (resolved === COURSEWARE_TASK_STATUS.SUCCEEDED) return 'SUCCEEDED'
  if (resolved === COURSEWARE_TASK_STATUS.CANCELED || failed.value) return 'CANCELED'
  return resolved === COURSEWARE_TASK_STATUS.QUEUED ? 'QUEUED' : 'RUNNING'
}
  ...
  return indexes.map((partIndex) => {
    ...
    return {
      ...
      title:
        ...
        `PPT ${partIndex}`,
      progress: item ? seriesItemProgress(item) : pageProgress,
      state: item ? partState(item) : undefined
    }
  })
})
...
const activePages = computed(() =>
  (props.task?.generation?.pages || []).filter(
    (page) => page.partIndex === activePart.value?.partIndex
  )
)
...
const selectPart = (partIndex: number) => {
  ...
  const currentIndex = generationParts.value.findIndex(
    (part) => part.partIndex === activePart.value?.partIndex
  )
  const nextIndex = generationParts.value.findIndex((part) => part.partIndex === partIndex)
  partSwitchDirection.value = nextIndex >= currentIndex ? 'next' : 'previous'
  partSwitching.value = true
  selectedPartIndex.value = partIndex
  previewRequest += 1
  previewOpen.value = false
  preview.value = undefined
  if (partSwitchTimer) clearTimeout(partSwitchTimer)
  partSwitchTimer = setTimeout(() => {
    partSwitching.value = false
    partSwitchTimer = undefined
  }, 420)
}
...
const quizPagePattern = /quiz|homework|练习|课后题|随堂练习/i
const isQuizPage = (page: CoursewarePageStateVO) => {
  const candidate = page as CoursewarePageStateVO & {
    type?: string
    sceneType?: string
    outlineType?: string
  }
  return [candidate.type, candidate.sceneType, candidate.outlineType, page.outlineId, page.title]
    .filter(Boolean)
    .some((value) => quizPagePattern.test(String(value)))
}
...
const quizCardStatus = computed(() => {
  if (!activePages.value.length || activePages.value.some((page) => page.status !== 'completed'))
    return 'queued'
  const status = (
    activeSeriesItem.value?.homeworkGenerationStatus || props.task?.homeworkGenerationStatus || ''
  ).toLowerCase()
  if (status === 'failed' || status === 'canceled') return 'failed'
  if (status === 'running' || status === 'succeeded') return status === 'succeeded' ? 'completed' : status
  if (status === 'imported' || status === 'completed') return 'completed'
  return 'queued'
})
const quizCardStatusLabel = computed(() => {
  ...
  return labels[quizCardStatus.value]
})
const quizCardHint = computed(() => {
  if (quizCardStatus.value === 'queued') return '当前子课件页面完成后，将开始生成课后练习。'
  if (quizCardStatus.value === 'running') return '页面已完成，正在生成课后练习。'
  if (quizCardStatus.value === 'completed') return '课后练习已完成，可点击页面预览查看。'
  return activeSeriesItem.value?.homeworkGenerationMessage || '课后练习生成失败，可稍后重试。'
})
...
watch(
  key,
  async (value) => {
    const preserveOutlineEdits =
      Boolean(draft.outlines.length) && Boolean(props.task?.partSelection?.parts?.length)
    if (!preserveOutlineEdits && draftTaskId.value !== props.task?.id)
      Object.assign(draft, createStudioDraft(props.task))
    draftTaskId.value = props.task?.id
    editingSection.value = ''
    validationError.value = ''
    if (props.task?.id && !preserveOutlineEdits) {
      try {
...
function confirm() {
  if (props.busy || !waitingForConfirmation.value) return
  cancelAutoAdvance()
  if (props.autoConfirm) autoAdvancedKey = key.value
  validationError.value = ''
  if (
