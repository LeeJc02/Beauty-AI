 ...
 *
 * 原型用 `group-hover/monthly-points:block` 控制气泡显隐；命名分组变体在 unocss 0.58
 * 不可靠，这里用 scoped css 实现。定位类（默认居中 / 传 `tooltipClass` 左对齐）二选一，
 * 避免原型里 twMerge 消解 class 冲突那套机制在 unocss 下失效。
 */
...
<template>
  <span
    class="monthly-points relative inline-flex items-center justify-center align-middle normal-case tracking-normal"
  >
    <button
      type="button"
      aria-label="当月积分计算公式"
