...
import { recommendedQuestionAnswer } from './generationTask'
...
export function studioDraftKey(task?: CoursewareGenerationTaskVO | null): string {
  ...
  return JSON.stringify([
    task.id,
    task.partSelection?.parts?.length ? 'parts' : prompt?.summary ? 'summary' : 'interview',
...
export function createStudioDraft(task?: CoursewareGenerationTaskVO | null): StudioDraft {
  ...
  const knownScope = known.scope as { mustInclude?: string[] } | undefined
  return {
    answers: Object.fromEntries(
      (task?.promptEnhancement?.questions || []).map((question) => [
        question.id,
        recommendedQuestionAnswer(question)
      ])
    ),
    customAnswers: {},
    notes: {},
    brief: {
      title: summary?.title || text(known.title) || task?.title || '',
      audience: summary?.audience || text(known.audience),
      objective: summary?.objective || text(known.objective),
      mustInclude: [...(summary?.scope?.mustInclude || knownScope?.mustInclude || [])]
    },
