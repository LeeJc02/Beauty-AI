<template>
  ...
  >
    <div class="relative mx-auto h-full flex">
      ...
      <div
        class="relative flex-1 p-30px lt-sm:p-10px overflow-x-hidden overflow-y-auto beauty-login-panel"
      >
        <!-- 右上角的主题、语言选择 -->
        <div
        ...
        >
          ...
          <div class="flex items-center justify-end space-x-10px h-48px">
            <LocaleDropdown class="lt-xl:text-white" />
          </div>
        </div>
        <!-- 右边的登录界面 -->
...
const loginData = reactive({
  ...
  // 模板中保留了历史写法 `loginData.captchaEnable === 'true'`（boolean 与字符串比较恒为 false），
  // 为不改运行时取值，这里把声明类型放宽为 string | boolean
  captchaEnable: (import.meta.env.VITE_APP_CAPTCHA_ENABLE !== 'false') as string | boolean,
...
onMounted(() => {
  ...
  authUtil.clearLegacyLoginFormCache()
  getCookie()
  tryLogin()
})
</script>
