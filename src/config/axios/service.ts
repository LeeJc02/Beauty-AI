...
service.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    const currentLang = useLocaleStoreWithOut().getCurrentLocale.lang
    config.headers.lang = currentLang === 'zh-CN' ? 'cn' : currentLang
    // 设置租户
...
service.interceptors.response.use(
  async (response: AxiosResponse<any>) => {
    ...
    } else if (ignoreMsgs.indexOf(msg) !== -1) {
      // 如果是忽略的错误码，直接返回 msg 异常
      return Promise.reject(msg)
    } else if (code === 500) {
      const errorMsg = (response.config as any).returnErrorMsg && msg ? msg : t('sys.api.errMsg500')
      ElMessage.error(errorMsg)
      if ((response.config as any).returnErrorMsg) {
  ...
  (error: AxiosError) => {
    ...
    // 后台轮询在工作区展示重连状态；仍保留鉴权和业务错误的原有提示。
    const silentNetworkError = (
