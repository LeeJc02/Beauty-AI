...
/**
 * 订阅自定义审计需求列表变更（原 React 版 `useSyncExternalStore` 的 subscribe 参数）。
 * Vue 侧由 `@/beauty/composables/useRequirements` 的 `useRequirements()` 消费。
 * 返回值是取消订阅函数。
 */
export function subscribeRequirements(listener: () => void): () => void {
