# 数据审计（Supervisor 工作台）· v0.4.0（2026-09-16）

**Scope**: `src/lib/auditTools.ts`（工具白名单与工具实现）、`src/lib/auditHistory.ts`（历史周期视图）、`src/pages/training-inspection/AuditConsole.tsx`（对话 + 补问 + 工具轨迹 + 报告）、`src/pages/training-inspection/OverviewTab.tsx`（首屏顺序）、`src/pages/TrainingInspection.tsx` 与 `src/components/Layout.tsx`（页面与菜单改名「数据审计」）。

**参考文档**：`/Users/lee/Projects/SalesBoost/Supervisor/确定设计.md`、`待确定设计.md`。

**Checks**

- 页面与菜单改名：四个角色的侧栏入口都显示「数据审计」，`#training_inspection` 入口不变；标题区 eyebrow 改为 `DATA AUDIT AGENT`，副标题说明「Agent 用工具核对任务、人群、工时与规则口径」；标签页为「审计工作台 / 区域工时审计 / 自定义审计」。截图 `qa/data-audit-workbench-idle.png`。
- 首屏顺序：对话 Agent → 指标卡 → 最近一次审计（主要结论 + 四条结论 + 频次）→ 审计工作记录 + 自定义审计（xl 两列）。`npm run lint`、`npm run test:inspection`（57 项全绿）、`npx vite build` 通过。
- 条件补齐：问「最近培训情况怎么样？」先补问时间（本周 / 上周 / 下周）→ 范围（全国 / 四个区域）→ 关注方面（全部 / 工时 / 完成 / 数据完整度 / 重复 / 品类覆盖），每一步既能点选项也能打字（「上周」可直接命中）；答完才开跑。截图 `qa/data-audit-clarification.png`。
- 工具调用轨迹：8 个工具按 `tool_call_started → tool_call_completed` 逐条出现，工具卡展示工具名、归属（ADM / Supervisor）、权限码（如 `supervisor:scope:read`）、实参（周期 / 范围 / 维度）、`traceId`、扫描范围、耗时、结论、指标、可展开的明细表、口径说明与数据来源。截图 `qa/data-audit-tool-trajectory.png`。
- 报告：结论徽标（阻断）+ `report_completed` 事件 + 规则集版本 + 6 项指标 + 关键发现（规则编号 / 原因 / 证据 / 数据出处 / 看任务详情）+ 整改建议（注明不自动改任务）+ 数据来源与口径。截图 `qa/data-audit-report.png`。
- 保存类动作：报告上「保存审计记录」产出 `save_inspection_record` 卡片，记录编号跟着审计周期走（上周审计得到 `insp-20260907-…`），列明「是否修改任务：否」；「每周一自动审计」产出 `save_schedule` 卡片（结构化条件 + 飞书群消息草稿，注明由 ADM 发送）。
- 权限异常：切到「区域培训师主管」后问「雅加达北区这周的培训怎么样？」→ `query_scope` 返回 `PERMISSION_DENIED`，写明「当前账号是南区培训主管，只能查看雅加达南区与全国任务」、模型可见说明、不可重试 / 需要用户补充信息，并给出可问范围；不会绕过权限取数。
- 数据异常：`check_data_gaps` 返回 `DATA_FIELD_MISSING`（26 项任务中 16 项完整、8 类字段缺失、涉及 10 项任务），并提示补齐后可重跑。
- 单独调用工具：侧栏「工具白名单」可点任意工具单独执行，卡片带调用次数与最近一次结论，用来单独验证口径。
- 历史周期：审计「上周」得到完整结论（覆盖 24 项任务 / 28 名可计工时 BA、完成率 76%、逾期率 26%、考试平均 83 分、命中 44 条规则），而不是空结果；报告里的任务链接还原成原任务 id。
- 响应式：390×844 下 `document.documentElement.scrollWidth === innerWidth === 390`，无横向溢出；工具卡、报告与三栏侧栏按单列堆叠。截图 `qa/data-audit-mobile-390.png`。
- 控制台只有 favicon 404，无本次功能相关报错。

**Patches Made**

- 多周期审计会跨周期重复计数：结论只保留「该结论所属周期里真的有排期」的任务后，4 周期从 940 条降到按周可核对的量级；时间选项收敛为本周 / 上周 / 下周，对应设计里「定时汇报默认复盘最近一个完整周」。
- 报告上的保存类动作原先退回页面默认周期，已改为沿用报告自己的查询条件。
- 「有定义但会被误触发的补问」：`看看 / 帮我审` 这类词不再触发关注方面追问，条件齐全的问题直接开跑。
- 工具卡 / 报告的 React key 加上下标，消除同规则结论重复 key 的告警。

**Follow-up Polish**

- 多周期趋势对比、模型侧真实补问与飞书发送按《Supervisor 待确定设计》继续确认后再做。

final result: passed

---

# 自定义审计需求（Agent 创建）· v0.3.2（2026-09-14）
...
- 向导主流程：点「新建一个审计需求」→ 关注方面多选（重复布置 + 时间集中）→ 范围（雅加达南区）→ 周期（每周一 08:00）→ 起止（连续 4 周）→ 确认页显示默认名称并可改 → 创建成功自动切到新标签页；截图 `qa/requirement-wizard-aspects.png`、`qa/requirement-wizard-confirm.png`。
...
- 标签页：`审计总览 | 区域负担 | 雅加达南区综合每周审计`，内容为状态摘要（发现的问题 / 受影响的 BA / 人均分钟 / 最近一次运行）→ 现在关注到的问题 → 运行记录；截图 `qa/requirement-tab.png`、`qa/requirement-run-records.png`。
- 运行：周一 08:00 已过点，打开页面自动补跑一条批次（`lastAutoDay` 记录当天，刷新不会重复）；再点「立即审计」结论没变，只合并成一条并提示「结论和上次一样」。
...
- 数量上限：预置 6 条后再创建，Agent 回复「最多同时保留 6 个自定义审计，请先在标签页上关掉不用的」，本地记录仍为 6 条；截图 `qa/requirement-limit.png`。
...
- 对话面板标题改为「和审计Agent对话」；「最近一次审计」右上角显示真实频次「每 30 分钟自动审计一次，有变化才记新批次（想改对 Agent 说「改成每 2 小时一次」）」。
- 区域负担任务行：「调整」已改成「查看详情」，点击弹出只读任务详情（练习 · 南区 VIP 顾客练习冲刺 · v1 · 选修 · 进行中；品类 异议处理/新品；任务类型 / 负责人 / 开始截止 / 频次 / 单次时长 / 创建发布 / 下发范围 / 命中人数；本周排期 09/20 · 5 次 · 50 分钟；内容资源；关联任务；执行情况 15 次应完成/9 次已完成 60%；人员名单）。行内不再出现「调整」。截图 `qa/task-detail-dialog.png`。
- 顶部操作自解释：「统计周期」标签 + 三个控件的悬停说明；导出按钮改名「导出工作记录」并实测下载成功（`培训审计工作记录-2026-09-14.txt`，约 23.6k 字符，含最近批次 / 批次记录 / 任务清单 / 结论明细 / 数据完整度 / 处置记录），toast 提示文件名与去向；入口改名「数据来源」，弹窗列出同步过来的任务与缺字段，可跳原任务。截图 `qa/inspection-header-clarified.png`、`qa/inspection-data-intake.png`。
- 去掉重复的状态行：顶部「最近审计 09/14 …（自动）· 26 项任务 · 9 个批次 · 本周期 26 项任务有问题」已删除（`.inspection-runbar` 只在本地存档异常时出现），最近一次的「自动审计 / 时间 / 任务数 / 执行人 / 结论没变，已连续 N 次」移到「最近一次审计」卡片右上角，与频次提示同排；卡片正文只剩主要结论与四条结论，不再重复。截图 `qa/inspection-runsbar-cleaned.png`。
...
- 频次对话：预设「自动审计多久跑一次？」回答当前设置；输入「把审计频率改成每 2 小时一次」当场生效——卡片提示变成「每 2 小时自动审计一次」，本地存档 `policy.autoRunMinutes = 120`，刷新后仍是 2 小时；截图 `qa/inspection-cadence.png`。
...
- 定时器从写死 60 秒改成按 `policy.autoRunMinutes` 递归排期；「每分钟自动审计」这类与策略不符的文案全部换掉。
- 引擎删除 C4「角色不适用」；新增 G 类规则 G1「品类覆盖不完整」，并接入总览结论分组、Agent 说法（「N 人所在的区域这期没排到这个品类」）与自定义审计的问题清单。
...
# 培训审计工作台 v0.2.0（2026-09-10）

**Scope**: `src/pages/TrainingInspection.tsx` 与 `src/pages/training-inspection/`（审计总览 / 任务体检 / 影响分析 / 处置中心 / 风险卡 / 模拟面板）。
...
- 顶部状态条移除角色胶囊、「本地演示数据」标记与「重置演示数据」按钮，仅保留审计时间与风险计数，本地存储异常时才显示提示。
...
- 标签页：只保留「审计总览 / 任务体检」，影响分析与处置中心入口消失；任务体检内「按当前模拟进入处置」仍能打开弹窗并提交。
- 首屏对话 Agent：预设问题（最近审计了哪些任务 / 现在有哪些任务需要我处理 / 风险最高的任务 / 哪些任务数据不全 / 上次审计到现在有什么变化 / 南区现在什么情况）逐条给出结论、证据与「工作记录批次」引用，引用里的任务可跳任务体检。
- 审计工作记录：批次默认折叠，展开后显示执行人、覆盖周期、本次变化（新增 / 解除 / 等级变化）与逐任务结论（任务名、类型、负责人、等级、命中规则号），每行可跳任务体检或原任务。
- 任务覆盖清单：按类型 / 结论 / 只看命中 / 区域筛选，行内展示最近审计时间、负责人、影响人数与区域、命中规则；展开行直接渲染风险卡（证据、建议、调整、处置）。
...
- 运行条「N 项审计结论」改为「本周期 N 条规则结论」，与指标卡口径一致。
- 待审批例外从处置中心标签页迁到处置弹窗顶部，审批人显示姓名而不是账号 ID，避免移除标签页后例外无法审批。

**Review Adjustments（2026-09-11 预览批注 · 措辞精简）**

- 页面标题「培训审计 · 审计工作记录」→「培训审计」，副标题压成一句；eyebrow 的「观察模式 · 只建议不修改」→「只建议 · 不改数据」。
- 状态条、Agent 卡片、批次记录、任务清单的说明各压到一行；「任务覆盖清单」→「审计过的任务」，筛选「只看命中的任务」→「只看有问题的」，「未命中规则」→「没问题」，「结论未变化」→「没变化」。
...
- 后端接入后，批次由服务端生成并按角色下发，浏览器不再承担审计调度。
...
- 口径行固定在表格上方：统计周期、区域容量（未确认区域标注「默认」）、单日 60 分钟阈值；数值与审计总览、Agent 回答同源（`regionAggregates`）。
...
- 从审计总览「最近一次审计」结论里点「南区新品卖点巩固」，页面切到区域负担、自动进入雅加达南区并高亮 `south-study` 行。
- 任务行点「调整」打开处置弹窗（标题「处置 · 南区 VIP 顾客练习冲刺」），未提交任何数据；浏览器控制台无报错。

**审计总览指标卡（业务口径，2026-09-14）**
...
- 顶部状态行：`最近审计 09-14 02:04（自动）· 26 项任务 · 9 个批次 · 本周期 26 项任务有问题`，不再出现「条结论」。
...
**向创建者发送调整建议（2026-09-15）**

**Scope**: `src/pages/training-inspection/TaskDetailDialog.tsx`（详情弹窗右下角按钮 + 站内信流程）、`src/lib/adjustmentSuggestion.ts`（收件人与草稿生成）。

**Checks**

- 弹窗里已经没有「调整任务」按钮（`getByRole("button", { name: "调整任务" })` 计数为 0），右下角只有「向创建者发送调整建议」（外部接入任务同时保留「打开原任务」）。
- 点按钮后先出现「Agent 正在根据审计结论生成调整建议…」的生成态，约 650ms 后展开草稿卡：标题「关于「南区 VIP 顾客练习冲刺」的调整建议」、收件人 `Fitriani（雅加达南区负责人 · 任务创建者）`、三条问题（`雅加达南区人均负荷超过容量 · 12 人这周任务量超了` / `09/20 单日负荷超过 60 分钟 · 12 人有一天任务排得太满` / `雅加达南区完成率偏低且负荷超容量`，同一说法只出现一次、日期写成 09/20）、建议动作两条、影响范围一行；截图 `qa/task-adjustment-suggestion.png`。
- 点「发送站内信」后草稿替换成绿色回执：`已发送站内信给 Fitriani（雅加达南区负责人 · 任务创建者）· 09/15 15:22`，并说明「…已进入「消息中心 › 我的消息」（原型演示，不真的发送）」；点「知道了」回到初始态。截图 `qa/task-adjustment-sent.png`。
- 关闭弹窗再打开，发送态自动复位（不会残留上一封草稿）。
- 原型说明：收件人取 `task.createdByName`（演示数据里总部任务 → Sarah Lee，南区 → Fitriani，北区 → Dewi，巴厘岛 → Putri，泗水 → Rina），取不到就落到任务负责人；发送只改界面状态，不写任务数据、不调后端。浏览器控制台无报错。
